...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    //HighScore?
    var HighScore = 0
    var Player = SKSpriteNode(imageNamed: "YourCar.png")
    ...
    override func didMove(to view: SKView) {
        ...
        if (lives == 3){
            LivesImage.image = UIImage(ciImage: "lives3.png")
        }
        if (lives == 2){
            LivesImage.image = UIImage(ciImage: "lives2.png")
        }
        if (lives == 1){
            LivesImage.image = UIImage(ciImage: "lives1.png")
        }
        self.addChild(LivesImage)
        LivesImage.center = CGPoint(x: view.frame.size.width / 2, y: view.frame.size.width / 5)
    ...
    func  CollisionWithPerson(Enemy:SKSpriteNode, Person:SKSpriteNode){
        ...
        if (lives == 0){
            ...
            if let Escene = SKScene(fileNamed: "EndScene") as? EndScene {
                // Set the scale mode to scale to fit the window
                Escene.scaleMode = .aspectFill
                Escene.HighSE = HighScore
                // Present the scene
                view!.presentScene(Escene)
            }
    ...
    func SpawnBullets(){
        let Bullet = SKSpriteNode(imageNamed: "road.png")
        Bullet.zPosition = -5
    ...
    func SpawnEnemies(){
        var Enemy = SKSpriteNode(imageNamed: "EnemyCar.png")
